...
const wouldHaveMatch = (
  ...
  const hypothetical = { ...currentSelected, [String(optionId)]: String(valueId) };
...
const buildSelectedOptions = (
  ...
  options.forEach((option) => {
    ...
    const chosenValue = option.values.find((value) => String(value.id) === String(chosenId));
...
export function ProductVariantPicker({
  ...
  const notify = (nextSelected: SelectedChoiceMap, nextTextInputs: SelectedTextMap) => {
    onVariantChange(
  ...
  const handleSelect = (optionId: string | number, valueId: string | number) => {
  ...
  const allSelected = nonInputOptions.every((option) => selected[String(option.id)]);
  const matched = findMatchingVariant(variants, selected, textInputs);
  const showUnavailable =
    hasRealVariantCombinations(variants) && nonInputOptions.length > 0 && allSelected && !matched;
              ...
              {option.isRequired ? (
                <View className="rounded-full bg-red-50 px-2 py-0.5 dark:bg-red-900/20">
                  <Text className="font-sans text-xs font-medium text-red-600 dark:text-red-300">
                    {t('product_form.options.required', { defaultValue: 'Required' })}
                  </Text>
                </View>
              ) : null}
            </View>
            ...
            {option.type === 'input' ? (
              ...
              <View className="flex-row flex-wrap gap-2">
                {option.values.map((value) => {
                  const selectedItem = String(selected[optionKey]) === String(value.id);
                  const disabled =
                    !selectedItem && !wouldHaveMatch(variants, selected, option.id, value.id);
                  ...
                  return (
                    <Pressable
                      ...
                      className={`min-h-10 min-w-0 items-center justify-center rounded-lg border ${
                        isColor ? 'h-11 w-11 rounded-full p-1' : isImage ? 'w-20 p-1' : 'px-3 py-2'
                      } ${
                        selectedItem
                      ...
                      {isColor ? (
                        <View
                          ...
                          style={{ backgroundColor: value.meta.hex || '#cccccc' }}>
                          {selectedItem ? <View className="h-3 w-3 rounded-full bg-white shadow" /> : null}
                        </View>
                      ) : isImage ? (
                        <View className="items-center gap-1">
                          <View className="h-12 w-12 overflow-hidden rounded-md bg-gray-100 dark:bg-slate-700">
                            {value.meta.imageUrl ? (
                              <Image
                              ...
                              <View className="h-full w-full items-center justify-center">
                                <Feather name="image" color="#94a3b8" size={16} />
                              </View>
                            )}
                          </View>
                          <Text
