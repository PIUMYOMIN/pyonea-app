...
const cleanValueMeta = (
  optType: OptionType,
  meta: OptionValueDraft["meta"],
) => {
...
const mapOption = (option: UnknownRecord): ProductOptionDraft => ({
  ...
  values: Array.isArray(option.values)
    ? option.values.filter(isRecord).map(mapValue)
    : [],
});
...
function NativeColorPicker({
  ...
  const display = isValidHex(normalizeHex(value))
    ? normalizeHex(value)
    : "#000000";
      ...
      <View className="min-w-0 flex-1 gap-2 lg:flex-row">
        <TextInput
          ...
          onChangeText={(label) =>
            onChange({
              ...value,
              label,
              value: value.value ? value.value : slugify(label),
            })
          }
          placeholder={t(
            "product_form.options.value_label_placeholder",
            "Label (e.g. Red)",
          )}
          placeholderTextColor="#9ca3af"
        ...
        <TextInput
          ...
          onChangeText={(nextValue) =>
            onChange({ ...value, value: slugify(nextValue) })
          }
          placeholder={t(
            "product_form.options.english_slug_placeholder",
            "English slug (e.g. red)",
          )}
          placeholderTextColor="#9ca3af"
          autoCapitalize="none"
          className={`min-w-0 flex-1 ${inputClassName}`}
        ...
        {optType === "color" && (
          <View className="min-w-0 flex-1 flex-row items-center gap-2">
            <WebColorInput
              ...
              onChange={(hex) =>
                onChange({ ...value, meta: { ...value.meta, hex } })
              }
            />
            <NativeColorPicker
              label={pickColourLabel}
          ...
          <View className="min-w-0 flex-1 flex-row items-center gap-2">
            ...
            <TextInput
              ...
              onChangeText={(imageUrl) =>
                onChange({
                  ...value,
                  meta: { ...value.meta, image_url: imageUrl },
                })
              }
              placeholder={t(
                "product_form.placeholders.image_url",
                "https://…/image.jpg",
              )}
              placeholderTextColor="#9ca3af"
              autoCapitalize="none"
...
function OptionBlock({
  ...
  const typeHint = typeInfo
    ? t(`product_form.options.types.${typeInfo.hintKey}`, "")
    : "";
    ...
    <View className="overflow-hidden rounded-xl border border-gray-200 dark:border-slate-600">
      <View className="flex-row flex-wrap items-center gap-2 bg-gray-50 px-3 py-3 dark:bg-slate-800 sm:gap-3 sm:px-4">
        ...
        <TextInput
          ...
          placeholder={t(
            "product_form.options.option_name_placeholder",
        ...
        <Pressable
          onPress={() =>
            onChange({ ...option, is_required: !option.is_required })
          }
        ...
        <Pressable
          onPress={() => setOpen((current) => !current)}
          className="h-8 w-8 flex-shrink-0 items-center justify-center"
        >
          <Feather
...
export function ProductOptionsEditorNative({
  ...
  const validate = () => {
    ...
    for (let index = 0; index < options.length; index += 1) {
      ...
      if (!option.name.trim()) {
        return t(
          "product_form.options.error_missing_name",
          "Option #{{number}} is missing a name. Fill it in or remove it.",
          {
            number: index + 1,
          },
        );
      ...
      if (option.type !== "input" && option.values.length === 0) {
        return t(
          "product_form.options.error_needs_value",
      ...
      for (const value of option.values) {
        if (!value.label.trim()) {
          return t(
            "product_form.options.error_value_label",
            'All values in option "{{name}}" need a label.',
            {
              name: option.name,
            },
          );
      ...
      {options.length === 0 ? (
        <View className="items-center rounded-xl border-2 border-dashed border-gray-300 py-10 dark:border-slate-600">
          ...
          <Text className="mt-1 font-sans text-xs text-gray-400 dark:text-slate-500">
            {t(
              "product_form.options.empty_hint",
              'Click "Add Option" to define Color, Size, etc.',
            )}
          </Text>
        </View>
      ) : (
      ...
      {success ? (
        <View className="flex-row items-start gap-2 rounded-lg border border-green-200 bg-green-50 px-3 py-2 dark:border-green-700 dark:bg-green-900/20">
          <Feather
            ...
            style={{ marginTop: 2 }}
          />
          <Text className="min-w-0 flex-1 font-sans text-sm text-green-700 dark:text-green-400">
            {success}
          </Text>
        </View>
      ) : null}
