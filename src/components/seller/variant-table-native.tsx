...
function VariantCard({
  ...
  return (
    ...
    >
      ...
      {error ? (
        <Text className="mt-2 font-sans text-xs text-red-500">{error}</Text>
      ) : null}
...
export function VariantTableNative({
  ...
  const loadVariants = useCallback(async () => {
    ...
    try {
      ...
      setGlobalError(
        formatApiErrorMessage(
          err,
          t("product_form.variants.load_failed", "Failed to load variants."),
        ),
      );
    } finally {
      setLoading(false);
  ...
  const saveVariant = async (variant: SellerManagedVariant) => {
    ...
    if (row.quantity === "" || Number.isNaN(quantity)) {
      setErrors((current) => ({
        ...current,
        [key]: t(
          "product_form.variants.valid_quantity",
    ...
    if (row.moq !== "" && row.moq != null) {
      ...
      if (Number.isNaN(moqVal)) {
        setErrors((current) => ({
          ...current,
          [key]: t(
            "product_form.variants.valid_moq",
            "MOQ must be a whole number or empty.",
          ),
        }));
        return;
      }
    ...
    try {
      ...
      setErrors((current) => ({
        ...current,
        [key]: formatApiErrorMessage(
          err,
          t("product_form.variants.save_failed", "Save failed."),
        ),
      }));
    } finally {
  ...
  const toggleActive = async (variant: SellerManagedVariant) => {
    ...
    setErrors((current) => ({ ...current, [key]: "" }));
    try {
      const nextActive = await toggleSellerProductVariant(
        productId,
        variant.id,
      );
      ...
      setVariants((current) =>
        current.map((item) =>
          String(item.id) === key ? { ...item, isActive: nextActive } : item,
        ),
      );
      setGlobalError("");
      onUpdated?.();
  ...
  const deleteVariant = (variant: SellerManagedVariant) => {
    ...
    const label =
      variant.label ||
      variant.optionValues.map((value) => value.label).join(" / ") ||
      "variant";
    Alert.alert(
      t("product_form.actions.delete", "Delete"),
      t("product_form.variants.delete_confirm", 'Delete variant "{{label}}"?', {
        label,
      }),
      [
                ...
                setErrors((current) => ({
                  ...
                  [key]:
                    err instanceof Error
                      ? err.message
                      : t(
                          "product_form.variants.delete_failed",
                          "Delete failed.",
                        ),
                }));
                await loadVariants();
  ...
  const generateVariants = async () => {
    ...
    if (!genDefaults.price.trim()) {
      setGlobalError(
        t(
          "product_form.variants.default_price_required",
          "Please enter a default price before generating.",
        ),
      );
      return;
    }
    ...
    if (Number.isNaN(genPrice)) {
      setGlobalError(
        t(
          "product_form.variants.valid_default_price",
    ...
    if (Number.isNaN(genQty) || genQty < 0) {
      setGlobalError(
        t(
          "product_form.variants.valid_default_quantity",
          "Please enter a valid default quantity (0 or more).",
        ),
      );
      return;
    }
    ...
    try {
      ...
      setGlobalError(
        formatApiErrorMessage(
          err,
          t("product_form.variants.generation_failed", "Generation failed."),
        ),
      );
    } finally {
      setGenerating(false);
  ...
  const totalLabel = useMemo(
    () =>
      variants.length > 0
        ? t("product_form.variants.total", "({{count}} total)", {
  ...
  return (
    <View className="gap-4">
      <View className="flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <View className="min-w-0 flex-1">
          ...
          <Text className="mt-0.5 font-sans text-sm text-gray-500 dark:text-slate-400">
            {t(
              "product_form.variants.subtitle",
              "Each row is one purchasable combination. Set price and stock per variant.",
            )}
          </Text>
        </View>
        <View className="flex-row flex-nowrap items-center justify-between gap-2 self-stretch sm:w-auto sm:self-auto sm:justify-start">
          <Pressable
            onPress={() => {
          ...
          >
            ...
            <Text className="font-sans text-sm font-semibold text-white">
              {variants.length > 0
                ...
                : t(
                    "product_form.variants.generate_variants",
                    "Generate Variants",
                  )}
            </Text>
          </Pressable>
        ...
        <View className="gap-3 rounded-xl border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-900/20">
          ...
          <View className="gap-3 md:grid md:grid-cols-3">
            <EditableField
              ...
              onChange={(value) =>
                setGenDefaults((current) => ({ ...current, price: value }))
              }
              keyboardType="numeric"
            />
            <EditableField
              ...
              onChange={(value) =>
                setGenDefaults((current) => ({ ...current, quantity: value }))
              }
              keyboardType="numeric"
            />
            <EditableField
              ...
              onChange={(value) =>
                setGenDefaults((current) => ({ ...current, moq: value }))
              }
              keyboardType="numeric"
            />
          </View>
      ...
      {globalError ? (
        <View className="flex-row items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 dark:border-red-700 dark:bg-red-900/20">
          ...
          <Text className="min-w-0 flex-1 font-sans text-sm text-red-600 dark:text-red-400">
            {globalError}
          </Text>
        </View>
      ) : null}
                ...
                {variants.map((variant) => {
                  ...
                  return (
                    ...
                    >
                      <View className="flex-row items-center">
                        ...
                        <View className="w-28 pr-3">
                          <TextInput
                            value={row.price}
                            onChangeText={(value) =>
                              updateEdit(variant.id, "price", value)
                            }
                            keyboardType="numeric"
                        ...
                        <View className="w-28 pr-3">
                          <TextInput
                            value={row.quantity}
                            onChangeText={(value) =>
                              updateEdit(variant.id, "quantity", value)
                            }
                            keyboardType="numeric"
                            className="rounded-lg border border-gray-200 bg-white px-2 py-1.5 font-sans text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
                          />
                        ...
                        <View className="w-28 pr-3">
                          <TextInput
                            value={row.quantity_unit}
                            onChangeText={(value) =>
                              updateEdit(variant.id, "quantity_unit", value)
                            }
                            className="rounded-lg border border-gray-200 bg-white px-2 py-1.5 font-sans text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
                          />
                        </View>
                        <View className="w-28 pr-3">
                          <TextInput
                            value={row.moq}
                            onChangeText={(value) =>
                              updateEdit(variant.id, "moq", value)
                            }
                            keyboardType="numeric"
                            className="rounded-lg border border-gray-200 bg-white px-2 py-1.5 font-sans text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
                          />
                        ...
                        <View className="w-28 pr-3">
                          <TextInput
                            value={row.sku}
                            onChangeText={(value) =>
                              updateEdit(variant.id, "sku", value)
                            }
                            className="rounded-lg border border-gray-200 bg-white px-2 py-1.5 font-sans text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
                          />
                        </View>
                      ...
                      {errors[key] ? (
                        <Text className="mt-2 font-sans text-xs text-red-500">
                          {errors[key]}
                        </Text>
                      ) : null}
                    </View>
              ...
              return (
                <VariantCard
                  ...
                  onChange={(field, value) =>
                    updateEdit(variant.id, field, value)
                  }
