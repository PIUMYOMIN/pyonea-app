...
export type SellerDiscountScope =
  | "all_products"
  | "specific_products"
  | "specific_categories";
...
export const getStoredAuthToken = () =>
  authToken || getWebStorage()?.getItem(TOKEN_STORAGE_KEY) || null;
...
const parseApiResponse = async <T>(
  ...
  if (text) {
    try {
      ...
      throw new ApiError(
        nonJsonResponseMessage(response.status),
        response.status,
      );
    }
  }
  ...
  if (!response.ok) {
    ...
    throw new ApiError(
      ...
      response.status,
      data.errors,
      getString(data.error) || undefined,
      data.data,
    );
  }
...
const mapNativeUser = (value: unknown): NativeUser => {
  ...
  const roleRecords = Array.isArray(user.roles)
    ? user.roles.filter(isRecord)
    : [];
...
const mapNotification = (notification: UnknownRecord): NativeNotification => {
  ...
  return {
    ...
    message: getString(
      data.message || notification.message || notification.title,
    ),
...
export async function fetchAdminSellerReviews(
  ...
  return getArrayPayload(payload)
    .filter(isRecord)
    .map((review, index) => mapAdminReview(review, index, "seller"));
}
...
export async function fetchAdminProductReviews(
  ...
  return getArrayPayload(payload)
    .filter(isRecord)
    .map((review, index) => mapAdminReview(review, index, "product"));
}
...
export async function updateAdminReviewStatus(
  ...
  await apiPost(
    `${base}/${encodeURIComponent(String(reviewId))}/${action}`,
    {},
    signal,
  );
}
...
const mapSellerStoreSummary = (store: UnknownRecord): SellerStoreSummary => ({
  ...
  bannerUrl: getNativeImageUrl(
    store.store_banner || store.banner || store.cover_image,
  ),
  ...
  verificationStatus: getString(
    store.verification_status || store.verificationStatus,
  ),
  documentsSubmitted: Boolean(
    store.documents_submitted ?? store.documentsSubmitted,
  ),
  documentsSubmittedAt: getString(
    store.documents_submitted_at || store.documentsSubmittedAt,
  ),
  ...
  registrationNumber: getString(
    store.business_registration_number || store.registration_number,
  ),
  ...
  businessRegistrationDocumentUrl: getNativeImageUrl(
    store.business_registration_document,
  ),
  taxRegistrationDocumentUrl: getNativeImageUrl(
    store.tax_registration_document,
  ),
  ...
  setupProgress: isRecord(store.setup_progress)
    ? store.setup_progress
    : undefined,
  ...
  preferredPaymentMethod: getString(
    store.preferred_payment_method,
    "bank_transfer",
  ),
  ...
  businessHours: isRecord(store.business_hours)
    ? store.business_hours
    : undefined,
  sellerTier: (() => {
    const tier = getString(
      store.seller_tier || store.sellerTier,
      "bronze",
    ).toLowerCase();
    return tier === "silver" || tier === "gold" ? tier : "bronze";
  })(),
  deliveredOrdersCount: getNumber(
    store.delivered_orders_count ??
      store.deliveredOrdersCount ??
      store.delivered_orders,
  ),
...
const mapSellerOnboardingStatus = (
  status: UnknownRecord,
): SellerOnboardingStatus => ({
  needsOnboarding: Boolean(status.needs_onboarding ?? status.needsOnboarding),
  onboardingComplete: Boolean(
    status.onboarding_complete ?? status.onboardingComplete,
  ),
  setupProgress: isRecord(status.setup_progress)
    ? status.setup_progress
    : undefined,
});
...
export async function fetchSellerDashboardOverview(
  ...
  const storeData =
    isRecord(storePayload) && isRecord(storePayload.data)
      ? storePayload.data
      : storePayload;
  ...
  const statsData =
    isRecord(statsPayload) && isRecord(statsPayload.data)
      ? statsPayload.data
      : statsPayload;
  ...
  const onboardingData =
    isRecord(onboardingPayload) && isRecord(onboardingPayload.data)
      ? onboardingPayload.data
      : onboardingPayload;

  return {
    store: isRecord(storeData) ? mapSellerStoreSummary(storeData) : null,
    stats: isRecord(statsData)
      ? mapSellerDashboardStats(statsData)
      : emptySellerDashboardStats,
    onboarding: isRecord(onboardingData)
      ? mapSellerOnboardingStatus(onboardingData)
      : null,
  };
}
...
export function mapSellerSettingsPayload(
  data: UnknownRecord,
  ...
  return {
    ...
    preferred_payment_method: getString(
...
const normalizeUploadMimeType = (type: string) =>
...
export async function appendNativeFile(
  form: FormData,
  field: string,
  ...
  if (Platform.OS === "web") {
    ...
    const webMimeType = normalizeUploadMimeType(
      file.type || blob.type || mimeType,
    );
...
export async function uploadSellerStoreLogo(
  file: NativeUploadFile,
  signal?: AbortSignal,
): Promise<void> {
...
export async function uploadSellerStoreBanner(
  file: NativeUploadFile,
  signal?: AbortSignal,
): Promise<void> {
  const form = new FormData();
...
const mapSellerManagedProduct = (
  product: UnknownRecord,
): SellerManagedProduct => {
  ...
  const discountPercentage = getNumber(
    product.discount_percentage || product.effective_discount_pct,
  );
  const computedSalePrice =
    discountPrice ||
    (discountPercentage > 0
      ? priceValue - priceValue * (discountPercentage / 100)
      : 0);
  const isOnSale = Boolean(
    product.is_on_sale || discountPrice || discountPercentage,
  );
  ...
  return {
    ...
    name: getString(
      product.name || product.name_en || product.name_mm,
      "Unnamed product",
    ),
    ...
    categoryName: getString(
      category.name_en || category.name_mm || product.category_name,
      "Uncategorized",
    ),
    ...
    inStock:
      ...
      getNumber(product.total_stock ?? product.stock) > 0,
    totalStock: getNumber(product.total_stock ?? product.stock),
  };
};

const mapSellerProductLimitUsage = (
  usage: UnknownRecord,
): SellerProductLimitUsage => ({
...
const mapSellerProductReview = (
  ...
  return {
    ...
    buyerName: getString(
      user?.name || buyer?.name || review.user_name || review.buyer_name,
      "Anonymous",
    ),
    productName: getString(
      product?.name || product?.name_en || review.product_name,
      "Unknown product",
    ),
...
export async function fetchSellerProductReviews(
  ...
  const reviews = getArrayPayload(payload)
    .filter(isRecord)
    .map(mapSellerProductReview);
...
const mapSellerCustomerStats = (stats: UnknownRecord): SellerCustomerStats => {
  const totalRevenueValue = getNumber(
    stats.total_revenue || stats.totalRevenue,
  );
  const avgOrderValueValue = getNumber(
    stats.avg_order_value || stats.avgOrderValue,
  );
...
const mapSellerCustomer = (
  customer: UnknownRecord,
  index = 0,
): SellerCustomer => {
  const totalSpentValue = getNumber(
  ...
  return {
    ...
    phone: getString(
      customer.phone || customer.phone_number || customer.customer_phone,
    ),
    ...
    avgOrderValueValue,
    avgOrderValue: formatMMK(avgOrderValueValue),
    deliveredCount: getNumber(
      customer.delivered_count || customer.deliveredCount,
    ),
...
export async function fetchSellerCustomers(
  ...
  const payload = await apiGet(
    `/seller/customers?${params.toString()}`,
    signal,
  );
  const root = isRecord(payload) ? payload : {};
  const paginator =
    isRecord(root.data) && !Array.isArray(root.data) ? root.data : root;
  const customers = getArrayPayload(payload)
    .filter(isRecord)
    .map(mapSellerCustomer);
...
const mapSellerSalesTrendPoint = (
  item: UnknownRecord,
  compactLabel: Intl.DateTimeFormat,
): SellerSalesTrendPoint => {
  ...
  const label =
    parsedDate && !Number.isNaN(parsedDate.getTime())
      ...
      : date || "N/A";
  const salesValue = getNumber(
    item.revenue || item.sales || item.total_revenue,
  );
...
const mapSellerTopProductSale = (
  product: UnknownRecord,
  index = 0,
): SellerTopProductSale => {
  const revenueValue = getNumber(
    product.total_revenue || product.revenue || product.sales_revenue,
  );
  ...
  return {
    ...
    sales: getNumber(
      product.total_sold || product.sales || product.quantity_sold,
    ),
    revenueValue,
...
export async function fetchSellerSalesReports(
  signal?: AbortSignal,
): Promise<SellerSalesReportsData> {
  const [summaryPayload, topProductsPayload] = await Promise.all([
    apiGet("/seller/sales-summary", signal),
    apiGet("/seller/top-products", signal),
  ]);
  const summaryRoot =
    isRecord(summaryPayload) && isRecord(summaryPayload.data)
      ? summaryPayload.data
      : summaryPayload;
  const summaryData = isRecord(summaryRoot) ? summaryRoot : {};
  const sales = isRecord(summaryData.sales) ? summaryData.sales : {};
  const customers = isRecord(summaryData.customers)
    ? summaryData.customers
    : {};
  const trend = Array.isArray(summaryData.recent_trend)
    ? summaryData.recent_trend.filter(isRecord)
    : [];
  ...
  const monthlyData = trend.map((item) =>
    mapSellerSalesTrendPoint(item, monthFormatter),
  );
  const weeklyData = trend
    .slice(-7)
    .map((item) => mapSellerSalesTrendPoint(item, dayFormatter));
  const topProducts = getArrayPayload(topProductsPayload)
    .filter(isRecord)
    .map(mapSellerTopProductSale);
  const totalSalesValue = getNumber(
    sales.total_revenue || summaryData.total_revenue,
  );
...
const mapSellerFinancialSummary = (
  summary: UnknownRecord,
  ...
  const totalCommissionPendingValue = getNumber(
    summary.total_commission_pending,
  );
  const totalCommissionConfirmedValue = getNumber(
    summary.total_commission_confirmed,
  );
  ...
  const totalDeliveryFeesPendingValue = getNumber(
    summary.total_delivery_fees_pending,
  );
  const totalDeliveryFeesConfirmedValue = getNumber(
    summary.total_delivery_fees_confirmed,
  );
  ...
  return {
    ...
    wallet: isRecord(summary.wallet)
      ? mapSellerWalletSummary(summary.wallet)
      : undefined,
  };
};

const mapSellerFinancialTrendPoint = (
  item: UnknownRecord,
): SellerFinancialTrendPoint => ({
...
const mapSellerFinancialOrder = (
  order: UnknownRecord,
  index = 0,
): SellerFinancialOrder => {
...
export async function fetchSellerFinancialReport(
  ...
  const payload = await apiGet(
    `/seller/financial-report?${params.toString()}`,
    signal,
  );
  const data =
    isRecord(payload) && isRecord(payload.data) ? payload.data : payload;
  ...
  const trend = Array.isArray(record.trend)
    ? record.trend.filter(isRecord)
    : [];
  const orders = Array.isArray(record.orders)
    ? record.orders.filter(isRecord)
    : [];
...
const mapAdminFinancialOrderItem = (
  item: UnknownRecord,
  index = 0,
): AdminFinancialOrderItem => {
...
export async function fetchAdminFinancialReport(
  ...
  const payload = await apiGet(
    `/admin/financial-report?${params.toString()}`,
    signal,
  );
  const data =
    isRecord(payload) && isRecord(payload.data) ? payload.data : payload;
...
const normalizeIdArray = (value: unknown): (string | number)[] => {
  ...
  return value
    .map((item) => {
      ...
      if (isRecord(item))
        return getString(item.id || item.product_id || item.category_id);
      return "";
    })
    .filter(Boolean);
};
...
const normalizeDiscountScope = (value: unknown): SellerDiscountScope => {
  ...
  return ["all_products", "specific_products", "specific_categories"].includes(
    scope,
  )
    ? scope
    : "specific_products";
...
const mapSellerDiscount = (
  discount: UnknownRecord,
  index = 0,
): SellerDiscount => {
  const minOrderAmount =
    discount.min_order_amount == null
      ? null
      : getNumber(discount.min_order_amount);
  const maxUses =
    discount.max_uses == null ? null : getNumber(discount.max_uses);
  const maxUsesPerUser =
    discount.max_uses_per_user == null
      ? null
      : getNumber(discount.max_uses_per_user);
  ...
  return {
    ...
    applicableTo: normalizeDiscountScope(
      discount.applicable_to || discount.applicableTo,
    ),
    applicableProductIds: normalizeIdArray(
      discount.applicable_product_ids || discount.applicableProductIds,
    ),
    applicableCategoryIds: normalizeIdArray(
      discount.applicable_category_ids || discount.applicableCategoryIds,
    ),
...
export async function fetchSellerDiscounts(
  signal?: AbortSignal,
): Promise<SellerDiscount[]> {
...
export async function saveSellerDiscount(
  ...
  const response = discountId
    ? await apiPut(
        `/seller/discounts/${encodeURIComponent(String(discountId))}`,
        payload,
        signal,
      )
...
export async function toggleSellerDiscountStatus(
  discountId: string | number,
  signal?: AbortSignal,
): Promise<void> {
  await apiPatch(
    `/seller/discounts/${encodeURIComponent(String(discountId))}/toggle-status`,
    {},
    signal,
  );
}

export async function deleteSellerDiscount(
  discountId: string | number,
  signal?: AbortSignal,
): Promise<void> {
  await apiDelete(
...
const mapSellerCoupon = (coupon: UnknownRecord, index = 0): SellerCoupon => {
  const minOrderAmount =
    coupon.min_order_amount == null ? null : getNumber(coupon.min_order_amount);
  const maxUses = coupon.max_uses == null ? null : getNumber(coupon.max_uses);
  const maxUsesPerUser =
    coupon.max_uses_per_user == null
      ? null
      : getNumber(coupon.max_uses_per_user);
  const rawProductIds =
    coupon.applicable_product_ids ?? coupon.applicableProductIds;
  const applicableProductIds =
    rawProductIds == null ? null : normalizeIdArray(rawProductIds);
...
export async function saveSellerCoupon(
  ...
  const response = couponId
    ? await apiPut(
        `/seller/coupons/${encodeURIComponent(String(couponId))}`,
        payload,
        signal,
      )
...
export async function toggleSellerCouponStatus(
  couponId: string | number,
  signal?: AbortSignal,
): Promise<void> {
  await apiPatch(
    `/seller/coupons/${encodeURIComponent(String(couponId))}/toggle-status`,
    {},
    signal,
  );
}
...
export async function updateSellerManagedProductStatus(
  ...
  await apiPut(
    `/seller/products/${encodeURIComponent(String(productId))}`,
    { is_active: isActive },
    signal,
  );
}
...
const mapSellerProductCategory = (
  category: UnknownRecord,
): SellerProductCategory => ({
  id: getString(category.id),
  name: getString(
    category.name || category.name_en || category.name_mm,
    "Category",
  ),
...
const mapSellerBulkImportedProduct = (
  value: unknown,
): SellerBulkImportedProduct => {
...
const mapSellerBulkImportResult = (
  payload: unknown,
): SellerBulkImportResult => {
  const root = isRecord(payload) ? payload : {};
  const data = isRecord(root.data) ? root.data : root;
  return {
    ...
    errors: Array.isArray(data.errors)
      ? data.errors.map(mapSellerBulkImportError)
      : [],
    importedList: Array.isArray(data.imported)
      ? data.imported.map(mapSellerBulkImportedProduct)
      : [],
  };
};

export async function downloadSellerBulkImportTemplate(
  signal?: AbortSignal,
): Promise<Blob> {
  const response = await fetchWithTimeout(
    "/seller/products/bulk-import/template",
    {
      headers: buildHeaders(),
      signal,
    },
...
export async function fetchSellerProductForEdit(
  ...
  const payload = await apiGet(
    `/seller/products/${encodeURIComponent(String(productId))}/edit`,
    signal,
  );
...
const mapSellerManagedVariantOptionValue = (
  value: UnknownRecord,
): SellerManagedVariantOptionValue => {
...
const mapSellerManagedVariant = (
  variant: UnknownRecord,
): SellerManagedVariant => ({
  ...
  optionValues: Array.isArray(variant.option_values)
    ? variant.option_values
        .filter(isRecord)
        .map(mapSellerManagedVariantOptionValue)
    : [],
});
...
export async function fetchSellerProductVariants(
  ...
  const payload = await apiGet(
    `/seller/products/${encodeURIComponent(String(productId))}/variants`,
    signal,
  );
...
const mapSellerWholesaleTier = (tier: UnknownRecord): SellerWholesaleTier => ({
  ...
  pricePerUnit: getString(
    tier.price_per_unit || tier.price || tier.pricePerUnit,
  ),
...
export async function fetchSellerWholesaleTiers(
  ...
  const payload = await apiGet(
    `/seller/products/${encodeURIComponent(String(productId))}/wholesale-tiers`,
    signal,
  );
  return getArrayPayload(payload)
    .filter(isRecord)
...
export const getNativeImageUrl = (image: unknown): string | undefined => {
  ...
  if (Array.isArray(image)) {
    ...
    const primary = image.find(
      (item) =>
        isRecord(item) && (item.is_primary === true || item.isPrimary === true),
    );

    for (const item of primary
      ? [primary, ...image.filter((entry) => entry !== primary)]
      : image) {
...
const resolveProductImageFromRecord = (
  product: UnknownRecord,
): string | undefined => {
...
const normalizeDisplayStyle = (value: unknown): AnnouncementDisplayStyle => {
  ...
  return ["popup_card", "popup_banner", "page_banner"].includes(style)
    ? style
    : "popup_card";
};
...
const mapAnnouncement = (value: UnknownRecord): Announcement => ({
  ...
  displayStyle: normalizeDisplayStyle(
    value.display_style || value.displayStyle,
  ),
  ...
  bannerAspectRatio: getString(
    value.banner_aspect_ratio || value.bannerAspectRatio,
    "16:9",
  ),
  ...
  targetAudience: getString(
    value.target_audience || value.targetAudience,
    "all",
  ),
...
export async function createAdminAnnouncement(
  ...
  const payload = await apiPostForm(
    "/admin/announcements",
    await buildAnnouncementFormData(body),
    signal,
  );
  const data =
    isRecord(payload) && isRecord(payload.data) ? payload.data : payload;
...
export async function toggleAdminAnnouncement(
  ...
  await apiPatch(
    `/admin/announcements/${encodeURIComponent(String(id))}/toggle`,
    {},
    signal,
  );
}
...
const mapReportAttachment = (
  attachment: UnknownRecord,
  index = 0,
): ReportAttachment => ({
  id: getString(
    attachment.id || attachment.path || attachment.url,
    `attachment-${index}`,
  ),
  label: getString(
  ...
  url: getString(
    attachment.url ||
      attachment.file_url ||
      (attachment.path ? toStorageUrl(getString(attachment.path)) : ""),
  ),
});

const mapReportComment = (
...
const mapAdminReport = (report: UnknownRecord, index = 0): AdminReport => {
  ...
  return {
    ...
    firstResponseAt: getString(
      report.first_response_at || report.firstResponseAt,
    ),
...
const mapUserReport = (report: UnknownRecord, index = 0): UserReport => {
  const comments = Array.isArray(report.comments)
    ? report.comments.filter(isRecord)
    : [];
  const attachments = Array.isArray(report.attachments)
    ? report.attachments.filter(isRecord)
    : [];

  return {
    id: getString(report.id || report.ticket_id, `report-${index}`),
    ticketId: getString(
      report.ticket_id || report.ticketId || report.id,
      `REPORT-${index}`,
    ),
...
const mapBulkOrderProduct = (
  product: UnknownRecord,
  index = 0,
): BulkOrderProduct => {
  const seller = isRecord(product.seller) ? product.seller : undefined;
  const moq = Math.max(
    getNumber(product.moq || product.minimum_order_quantity, 1),
    1,
  );
  ...
  return {
    ...
    slug: getString(
      product.slug_en || product.slug || product.id,
      `bulk-product-${index}`,
    ),
    ...
    unitLabel: getString(
      product.quantity_unit || product.min_order_unit,
      "piece",
    ).slice(0, 20),
    moq,
    quantityStep,
...
const getProductEnvelope = (payload: unknown) => {
  if (!isRecord(payload)) return payload;
  if (isRecord(payload.data) && isRecord(payload.data.product))
...
const normalizeProductOptionMeta = (
  value: unknown,
): ProductOptionValue["meta"] => {
  ...
  return {
    ...
    imageUrl: getNativeImageUrl(
      record.image_url || record.imageUrl || record.image,
    ),
  };
};

const mapProductOptionValue = (
  value: UnknownRecord,
  index: number,
): ProductOptionValue => ({
...
const mapProductOption = (
  ...
  return {
    id: getString(
      option.id || option.option_id || option.name,
      `option-${index}`,
    ),
...
const getVariantOptionValueIds = (
  variant: UnknownRecord,
): (string | number)[] => {
  ...
  return optionValues
    .map((value) =>
      getString(value.value_id || value.id || value.option_value_id),
    )
    .filter(Boolean);
};
...
const mapProductVariant = (
  ...
  const quantity = getNumber(
    variant.quantity || variant.stock || variant.total_stock,
  );
  return {
    ...
    quantity,
    inStock:
      variant.in_stock === true || variant.in_stock === 1 || quantity > 0,
    isActive: variant.is_active !== false && variant.is_active !== 0,
    moq: variant.moq == null ? undefined : getNumber(variant.moq),
    quantityStep:
      variant.quantity_step == null
        ? undefined
        : getNumber(variant.quantity_step),
    imageUrl: getNativeImageUrl(
      variant.image || variant.image_url || variant.imageUrl,
    ),
    optionValueIds: getVariantOptionValueIds(variant),
  };
};

export const getProductApiId = (
  product: Pick<HomeProduct, "id" | "productId">,
...
export const mapHomeProduct = (
  ...
  const stock = getNumber(
    product.total_stock ?? product.quantity ?? product.stock,
  );
...
const mapHomeSeller = (seller: UnknownRecord, index = 0): HomeSeller => {
  ...
  return {
    ...
    rating: getString(
      seller.reviews_avg_rating || seller.average_rating || seller.rating,
      "0",
    ),
...
const mapSellerProfile = (
  ...
  return {
    ...
    products: getNumber(
      stats.active_products || stats.total_products || seller.products_count,
      homeSeller.products,
    ),
    reviews: getNumber(
      seller.reviews_count || stats.reviews_count || seller.review_count,
      homeSeller.reviews,
    ),
    rating: getString(
      seller.reviews_avg_rating ||
        stats.average_rating ||
        seller.average_rating,
      homeSeller.rating,
    ),
    bannerUrl: getNativeImageUrl(
      seller.store_banner || seller.banner || seller.cover_image,
    ),
...
export const computeReviewStatsFromReviews = (
  reviews: SellerReview[],
): SellerReviewStats =>
  reviews.reduce(
    (acc, review) => {
      const key =
        `star${Math.max(1, Math.min(5, Math.round(review.rating)))}` as keyof SellerReviewStats;
      acc[key] += 1;
      return acc;
    },
...
const mapSellerReview = (review: UnknownRecord, index = 0): SellerReview => {
  ...
  return {
    ...
    userId: getString(review.user_id || user?.id || buyer?.id, ""),
    author: getString(
      user?.name || buyer?.name || review.name || review.author,
      "Anonymous",
    ),
    rating: getNumber(review.rating),
    comment: getString(review.comment || review.review || review.body),
...
const mapSellerDeliveryArea = (
  ...
  shippingFee:
    area.shipping_fee == null ? "Free" : formatMMK(area.shipping_fee),
  ...
  freeShippingThreshold: area.free_shipping_threshold
    ...
    : "",
  estimatedDaysMin: getNumber(
    area.estimated_delivery_days_min || area.days_min,
    3,
  ),
  estimatedDaysMax: getNumber(
    area.estimated_delivery_days_max || area.days_max,
    5,
  ),
  estimatedDays:
    getString(area.estimated_delivery_days || area.delivery_days) ||
      ...
      return getArrayPayload(payload)
        ...
        .map((category, index) => {
          ...
          return {
            ...
            childrenCount: Array.isArray(category.children)
...
export async function fetchCategoryBrowser(
  signal?: AbortSignal,
...
export function flattenBrowserCategories(
...
export async function fetchCategoryDetail(
  ...
  return withDataCache(
    ...
    async (requestSignal) => {
      ...
      try {
        const payload = await apiGet(
          `/categories/${encodeURIComponent(String(match.id))}`,
          requestSignal,
        );
        const record =
  ...
  return {
    ...
    seller: seller
      ? {
          ...
          name: getString(
            seller.store_name || seller.business_name || seller.name,
            "Pyonea seller",
          ),
          slug: getString(
            seller.store_slug || seller.slug || seller.id,
          ...
          rating: getString(
            seller.average_rating || seller.reviews_avg_rating || seller.rating,
            "0",
          ),
          verified:
...
export async function submitProductReview(
  ...
  return {
    ...
    rating: getNumber(
      record.product_rating || data.product_rating || data.average_rating,
    ),
    reviewCount: getNumber(
      record.product_review_count ||
        data.product_review_count ||
        data.review_count,
    ),
    message: getString(
...
export async function fetchSellerProfile(
  ...
  const payload = await apiGet(
    `/sellers/${encodeURIComponent(slug)}?${params.toString()}`,
    signal,
  );
  const data =
    isRecord(payload) && isRecord(payload.data) ? payload.data : payload;
  ...
  const productItems = Array.isArray(productPayload.data)
    ? productPayload.data.filter(isRecord)
    : [];
  ...
  return {
    ...
    products: productItems.map((product, index) =>
      mapHomeProduct(
        // Inject seller context so mapHomeProduct can resolve the store name.
        // The seller profile endpoint returns products without a nested seller
        // object (the seller is already known), so without this the name falls
        // back to the generic 'Pyonea seller' placeholder.
...
export async function fetchSellerReviews(
  ...
  const payload = await apiGet(
    `/reviews/sellers/${encodeURIComponent(slug)}?${params.toString()}`,
    signal,
  );
  const envelope =
    isRecord(payload) && isRecord(payload.data) ? payload.data : payload;
  ...
  const currentPage = getNumber(
    meta.current_page || envelopeRecord.current_page,
    page,
  );
  const lastPage = getNumber(
    meta.last_page || envelopeRecord.last_page,
    currentPage,
  );
...
export async function fetchSellerDeliveryAreas(
  ...
  return withDataCache(
    ...
    async (requestSignal) => {
      ...
      return getArrayPayload(payload)
        .filter(isRecord)
        .map(mapSellerDeliveryArea);
    },
    signal,
  );
}

export async function fetchSellerDeliveryZones(
  signal?: AbortSignal,
): Promise<SellerDeliveryArea[]> {
...
const mapCartItem = (item: UnknownRecord, index = 0): CartItem => {
  const selectedOptions = isRecord(item.selected_options)
  ...
  const quantityStep = Math.max(
    getNumber(item.quantity_step || minOrder, 1),
    1,
  );
  ...
  return {
    ...
    slug: getString(
      item.slug || item.product_slug || item.slug_en || productId,
      productId,
    ),
    name: getString(item.name || item.product_name, "Cart item"),
    category: getString(item.category || item.category_name),
    seller: getString(
      item.seller_name || seller?.store_name || seller?.business_name,
    ),
    sellerSlug: getString(
      item.seller_slug || seller?.store_slug || seller?.slug,
    ),
    ...
    quantityStep,
    quantityUnit: getString(
      item.quantity_unit || item.unit || item.min_order_unit,
      "pcs",
    ),
...
const mapCartSummary = (
  ...
  const taxValue = getNumber(
    summary.tax,
    Math.round(subtotalValue * taxRate * 100) / 100,
  );
  const shippingFeeValue = getNumber(summary.shipping_fee);
  const totalValue = getNumber(
    summary.total,
    subtotalValue + taxValue + shippingFeeValue,
  );
...
const mapCartResult = (payload: unknown): CartResult => {
  ...
  const totalItems = getNumber(
    cartData.total_items ??
      nestedCart.total_items ??
      cartData.totalItems ??
      nestedCart.totalItems,
...
export async function fetchCheckoutFees(
  ...
  const payload = await apiGet(
    `/orders/checkout-fees?${params.toString()}`,
    signal,
  );
  const data = isRecord(payload) && isRecord(payload.data) ? payload.data : {};
  const sellers = Array.isArray(data.sellers)
    ? data.sellers.filter(isRecord)
    : [];
    ...
    sellers: sellers.map((seller) => {
      ...
      return {
        ...
        sellerName: getString(
          seller.seller || seller.seller_name || seller.store_name,
          "Seller",
        ),
        shippingFeeValue,
        shippingFee: formatMMK(shippingFeeValue),
...
export async function fetchCheckoutProfile(
  signal?: AbortSignal,
...
export async function fetchCheckoutSellerPolicies(
  ...
  const mapSellerPolicy = (
    ...
    const shippingPolicy = getString(
      seller.shipping_policy || seller.shippingPolicy,
    );
    if (!returnPolicy && !shippingPolicy) return null;

    return {
      sellerId: getString(
        seller.id || seller.user_id || fallbackSlug,
        fallbackSlug,
      ),
      sellerName: getString(
        seller.store_name ||
      ...
      slug: getString(
        seller.store_slug || seller.slug || fallbackSlug,
        fallbackSlug,
      ),
      returnPolicy,
      shippingPolicy,
  ...
  const fetchPoliciesBySlug = async (
      ...
      requestSlugs.map(async (slug) => {
        try {
          const payload = await withDataCache(
            ...
            DATA_CACHE_TTL.checkoutStatic,
            (cachedSignal) =>
              apiGet(`/sellers/${encodeURIComponent(slug)}`, cachedSignal),
            requestSignal,
          );
          const data =
            isRecord(payload) && isRecord(payload.data)
              ...
              : payload;
          const seller =
            isRecord(data) && isRecord(data.seller) ? data.seller : data;
    ...
    return policies.filter(
      (policy): policy is CheckoutSellerPolicy => policy !== null,
    );
  };
  ...
  try {
    return await withDataCache(
      ...
      async (requestSignal) => {
        ...
        const payload = await apiGet(
          `/sellers/policies?${params.toString()}`,
          requestSignal,
        );
        const data =
          isRecord(payload) && isRecord(payload.data) ? payload.data : payload;
...
const mapTrackedOrderItem = (
  ...
  return {
    ...
    imageUrl: getNativeImageUrl(
      item.image || product?.image || product?.images,
    ),
    priceValue,
    price: formatMMK(priceValue),
    quantity,
...
const mapTrackedDelivery = (delivery: UnknownRecord): TrackedDelivery => {
  const updates = Array.isArray(delivery.updates)
    ? delivery.updates.filter(isRecord)
    : [];
...
const mapTrackedOrder = (order: UnknownRecord): TrackedOrder => {
  ...
  const parsedAddress = parseMaybeJson(
  ...
  const subtotalAmountValue = getNumber(
...
const mapPaymentReceiptOrder = (order: UnknownRecord): PaymentReceiptOrder => {
  ...
  const shippingAddress = normalizeReceiptAddress(
  ...
  return {
    ...
    orderNumber: getString(
      data.order_number || data.number || data.id,
      "Order",
    ),
    ...
    taxRate: getNumber(
      data.tax_rate,
      subtotalAmountValue > 0 ? taxAmountValue / subtotalAmountValue : 0,
    ),
    totalAmountValue,
    subtotalAmount: formatMMK(subtotalAmountValue),
    shippingFee: formatMMK(shippingFeeValue),
    ...
    customer: {
      name: getString(
        user?.name || data.customer_name || shippingAddress.fullName,
      ),
      email: getString(
        user?.email || data.customer_email || shippingAddress.email,
      ),
      phone: getString(
        user?.phone || data.customer_phone || shippingAddress.phone,
      ),
    },
    shippingAddress,
    items: items.map(mapTrackedOrderItem),
...
export async function trackOrder(
  ...
  const payload = await apiGet(
    `/track/${encodeURIComponent(trimmed)}${suffix}`,
    signal,
  );
  const data =
    isRecord(payload) && isRecord(payload.data) ? payload.data : payload;
...
const mapBuyerProfile = (profile: UnknownRecord): BuyerProfile => ({
  ...
  emailVerifiedAt: getString(
    profile.email_verified_at || profile.emailVerifiedAt,
  ),
...
export async function resendBuyerVerificationEmail(
  signal?: AbortSignal,
): Promise<void> {
...
const extractEmailVerificationResult = (
  payload: unknown,
...
export async function confirmBuyerOrderDelivery(
  ...
  await apiPost(
    `/orders/${encodeURIComponent(String(orderId))}/confirm-delivery`,
    {},
    signal,
  );
}
...
const mapSellerOrderDelivery = (
  delivery: UnknownRecord,
): SellerOrderDelivery => {
  const courier = isRecord(delivery.platformCourier)
    ? delivery.platformCourier
    : isRecord(delivery.platform_courier)
      ? delivery.platform_courier
      : {};
  const feeValue = getNumber(
    delivery.platform_delivery_fee || delivery.delivery_fee,
  );
  ...
  return {
    ...
    assignedDriverPhone: getString(
      delivery.assigned_driver_phone || delivery.driver_phone,
    ),
    courierName: getString(courier.name || delivery.courier_name),
  };
};

const normalizeShippingAddress = (value: unknown): SellerOrderAddress =>
  normalizeSellerOrderAddress(value);

const mapSellerDeliveryOrder = (
  value: unknown,
  delivery: UnknownRecord,
): SellerDeliveryOrder => {
  ...
  return {
    ...
    orderNumber: getString(
      order.order_number || order.orderNumber || delivery.order_number,
      "N/A",
    ),
...
const mapSellerDeliveryUpdate = (
  update: UnknownRecord,
): SellerDeliveryUpdate => ({
...
const mapSellerDelivery = (delivery: UnknownRecord): SellerDelivery => {
  ...
  return {
    ...
    platformDeliveryFee: formatMMK(feeValue),
    pickupAddress: getString(delivery.pickup_address || delivery.pickupAddress),
    deliveryAddress: getString(
      delivery.delivery_address || order.deliveryAddress,
    ),
    packageWeight: getNumber(
      delivery.package_weight || delivery.weight || order.packageWeight,
      5,
    ),
    courierName: getString(
      courier.name || delivery.courier_name || delivery.assigned_driver_name,
    ),
    driverPhone: getString(
      delivery.assigned_driver_phone || delivery.driver_phone,
    ),
    proofImageUrl: getNativeImageUrl(
      delivery.delivery_proof_image || delivery.proof_image,
    ),
    ...
    feeSubmittedAt: getString(
      delivery.fee_submitted_at || delivery.feeSubmittedAt,
    ),
    feeConfirmedAt: getString(
      delivery.fee_confirmed_at || delivery.feeConfirmedAt,
    ),
    feeSubmissionNote: getString(
      delivery.fee_submission_note || delivery.feeSubmissionNote,
    ),
    feeConfirmationNote: getString(
      delivery.fee_confirmation_note || delivery.feeConfirmationNote,
    ),
...
const mapSellerManagedOrder = (order: UnknownRecord): SellerManagedOrder => {
  ...
  const shippingAddress = normalizeSellerOrderAddress(
    order.shipping_address || order.shippingAddress,
  );
  const items = Array.isArray(order.items) ? order.items.filter(isRecord) : [];
  const delivery = isRecord(order.delivery) ? order.delivery : undefined;
  ...
  return {
    ...
    customerName: getString(
      user?.name || order.customer_name || shippingAddress.fullName,
      "N/A",
    ),
    customerEmail: getString(
      user?.email || order.customer_email || shippingAddress.email,
    ),
    customerPhone: getString(
      user?.phone || order.customer_phone || shippingAddress.phone,
    ),
    shippingAddress,
...
const mapAdminManagedOrder = (order: UnknownRecord): AdminManagedOrder => {
  ...
  const commissionAmountValue = getNumber(order.commission_amount);
  const sellerPayoutValue = Math.max(
    0,
    base.subtotalAmountValue - commissionAmountValue,
  );
...
async function advanceAdminOrderStatus(
  ...
  while (current !== toStatus && guard++ < 10) {
    ...
    if (
      current === "processing" &&
      (toStatus === "shipped" || toStatus === "delivered")
    ) {
      await apiPost(`/orders/${id}/ship`, adminShipPayload(), signal);
      current = "shipped";
      continue;
...
const mapAdminManagedProduct = (
  ...
  return {
    ...
    sellerName: getString(
      seller?.name || seller?.store_name || product.seller_name,
    ),
...
const mapAdminProductVariants = (
  product: UnknownRecord,
): AdminProductVariant[] => {
  const raw =
    product.activeVariants || product.active_variants || product.variants;
  if (!Array.isArray(raw)) return [];
  ...
  return raw.filter(isRecord).map((variant, index) => {
    ...
    return {
      ...
      optionLabel:
        labels.join(" / ") ||
        `Variant #${getString(variant.id, String(index + 1))}`,
    };
  });
};
...
export async function approveAdminProduct(
  ...
  await apiPost(
    `/admin/products/${encodeURIComponent(String(productId))}/approve`,
    {},
    signal,
  );
}
...
export async function deleteAdminProduct(
  ...
  await apiDelete(
    `/admin/products/${encodeURIComponent(String(productId))}`,
    signal,
  );
}
...
export async function fetchAdminProductForEdit(
  ...
  const payload = await apiGet(
    `/admin/products/${encodeURIComponent(String(productId))}/edit`,
    signal,
  );
  return mapSellerProductForm(extractRecordPayload(payload));
}
...
export async function fetchAdminUsers(
  ...
  const users = getArrayPayload(payload)
    .filter(isRecord)
...
export async function fetchAdminSellers(
  ...
  const nested =
    isRecord(payload) && isRecord(payload.data) ? payload.data : payload;
  const rows =
    isRecord(nested) && Array.isArray(nested.data)
      ? nested.data
      : getArrayPayload(payload);
...
export async function updateAdminSellerStatus(
  ...
  if (newStatus === "approved") {
    await apiPut(
      `/admin/seller/${encodeURIComponent(String(sellerId))}/approve`,
      payload,
      signal,
    );
    return;
  ...
  if (newStatus === "suspended") {
    await apiPost(
      `/admin/seller/${encodeURIComponent(String(sellerId))}/suspend`,
      payload,
      signal,
    );
    return;
  ...
  if (newStatus === "active") {
    await apiPost(
      `/admin/seller/${encodeURIComponent(String(sellerId))}/reactivate`,
      payload,
      signal,
    );
    return;
...
const mapAdminVerificationSeller = (
  seller: UnknownRecord,
): AdminVerificationSeller => {
  const user = isRecord(seller.user) ? seller.user : undefined;
  ...
  const additionalRaw = Array.isArray(seller.additional_documents)
    ? seller.additional_documents
    : [];
  ...
  return {
    ...
    rejectionReason: getString(
      seller.rejection_reason || seller.document_rejection_reason,
    ),
    ...
    nrcVerificationStatus: getString(
      seller.nrc_verification_status,
      "unverified",
    ),
    ...
    businessRegistrationDocument: getString(
...
const mapAdminVerifiedSeller = (
  ...
  businessRegistrationDocumentUrl: getString(
    seller.business_registration_document_url,
  ),
  ...
  additionalDocuments: (Array.isArray(seller.additional_documents)
    ? seller.additional_documents
    : []
  )
    .filter(isRecord)
...
export async function fetchAdminVerificationReview(
  ...
  sellers: AdminVerificationSeller[];
  pagination: AdminSellersPagination;
}> {
  const params = new URLSearchParams();
  params.set("page", String(filters.page ?? 1));
  ...
  if (filters.documentStatus?.trim())
    params.set("status", filters.documentStatus.trim());

  const payload = await apiGet(
    `/admin/seller/verification-review?${params.toString()}`,
    signal,
  );
  const nested =
    isRecord(payload) && isRecord(payload.data) ? payload.data : payload;
...
export async function fetchAdminVerifiedSellers(
  ...
  const rows =
    isRecord(nested) && Array.isArray(nested.data)
      ...
      : getArrayPayload(payload);
  const summaryRaw =
    isRecord(payload) && isRecord(payload.summary) ? payload.summary : {};
  const meta =
    isRecord(payload) && isRecord(payload.meta)
      ? payload.meta
      : extractAdminSellersMeta(payload, filters.page ?? 1);
...
export async function fetchAdminVerifiedSellersExportCsv(
  ...
  const response = await fetchWithTimeout(
    `/admin/seller/verified-list/export?${params.toString()}`,
    {
      headers: buildHeaders(),
      signal,
    },
...
const mapAdminPlatformDelivery = (
  delivery: UnknownRecord,
): AdminPlatformDelivery => {
  ...
  const shipping = normalizeSellerOrderAddress(
    order.shipping_address || order.shippingAddress,
  );
  ...
  return {
    ...
    platformCourierId:
      courierId === null || courierId === undefined || courierId === ""
        ? null
        : getString(courierId),
    assignedDriverName: getString(
      delivery.assigned_driver_name || base.courierName,
    ),
...
const extractDeliveriesMeta = (
  ...
  if (
    isRecord(payload.data) &&
    ("current_page" in payload.data || "total" in payload.data)
  ) {
    return payload.data;
  }
  return {};
...
export async function fetchAdminCourierCandidates(
  signal?: AbortSignal,
): Promise<AdminCourierCandidate[]> {
...
export async function assignAdminDeliveryCourier(
  ...
  if (form.platform_courier_id)
    payload.platform_courier_id = form.platform_courier_id;

  await apiPost(
    `/deliveries/${encodeURIComponent(String(deliveryId))}/assign-courier`,
    payload,
    signal,
  );
}
...
export async function updateAdminDeliveryStatus(
  ...
  await apiPost(
    `/deliveries/${encodeURIComponent(String(deliveryId))}/status`,
    { status, notes },
    signal,
  );
}
...
const mapAdminDeliveryFee = (delivery: UnknownRecord): AdminDeliveryFee => {
  ...
  return {
    ...
    deliveryFeeStatus: getString(
      delivery.delivery_fee_status,
      "not_applicable",
    ),
...
export async function fetchAdminDeliveryFees(
  ...
  const deliveriesSource =
    isRecord(data.deliveries) && Array.isArray(data.deliveries.data)
...
const mapAdminPendingDeliveryFee = (
  delivery: UnknownRecord,
  index = 0,
): AdminPendingDeliveryFee => {
...
export async function markAdminContactMessageRead(
  ...
  await apiPut(
    `/admin/contact-messages/${encodeURIComponent(String(messageId))}/read`,
    {},
    signal,
  );
}
...
export async function deleteAdminContactMessage(
  ...
  await apiDelete(
    `/admin/contact-messages/${encodeURIComponent(String(messageId))}`,
    signal,
  );
}
  ...
  const body =
    status === "shipped"
      ? {
          ...
          shipping_carrier: "Self Delivery",
        }
      : {};
  await apiPost(
    `/orders/${encodeURIComponent(String(orderId))}/${endpoints[status]}`,
    body,
    signal,
  );
...
export async function setSellerOrderDeliveryMethod(
  ...
  const platformDeliveryFee =
    method === "platform" ? 5000 + packageWeight * 100 : 0;
  await apiPost(
...
export async function fetchSellerDeliveries(
  signal?: AbortSignal,
): Promise<SellerDelivery[]> {
...
export async function uploadSellerDeliveryProof(
  ...
  const payload = await apiPostForm(
    `/deliveries/${encodeURIComponent(String(deliveryId))}/proof`,
    form,
    signal,
  );
  const data =
...
const mapSellerRfqBuyer = (value: unknown): SellerRfqBuyer | undefined => {
  ...
  return {
    ...
    name: getString(
      value.name || value.full_name || value.company_name,
      "Buyer",
    ),
...
const mapSellerRfqQuote = (quote: UnknownRecord): SellerRfqQuote => {
  ...
  return {
    ...
    sellerName: getString(
      profile?.store_name || seller?.store_name || seller?.name,
      "Seller",
    ),
    unitPriceValue,
    totalPriceValue,
...
const mapSellerRfq = (rfq: UnknownRecord): SellerRfq => {
  ...
  const quotes = Array.isArray(rfq.quotes)
    ? rfq.quotes.filter(isRecord).map(mapSellerRfqQuote)
    : [];
  ...
  return {
    id: getString(rfq.id),
    rfqNumber: getString(
      rfq.rfq_number || rfq.rfqNumber,
      `RFQ-${getString(rfq.id, "N/A")}`,
    ),
    productName: getString(
      rfq.product_name || rfq.productName,
      "Product request",
    ),
    ...
    order: orderRecord
      ? {
          ...
          orderNumber: getString(
            orderRecord.order_number || orderRecord.orderNumber,
          ),
...
export async function fetchSellerReceivedRfqs(
  signal?: AbortSignal,
): Promise<SellerRfq[]> {
...
export async function fetchSentRfqs(
  signal?: AbortSignal,
): Promise<SellerRfq[]> {
...
export async function fetchSellerRfqDetail(
  ...
  const payload = await apiGet(
    `/rfq/${encodeURIComponent(String(rfqId))}`,
    signal,
  );
...
export async function submitSellerRfqQuote(
  ...
  await apiPost(
    `/rfq/${encodeURIComponent(String(rfqId))}/quotes`,
    body,
    signal,
  );
...
export async function cancelRfq(
  ...
  await apiPatch(
    `/rfq/${encodeURIComponent(String(rfqId))}/cancel`,
    {},
    signal,
  );
}
...
const mapBlogPost = (post: UnknownRecord, index = 0): BlogPost => {
  ...
  const excerptEn = stripHtml(
    post.excerpt_en || post.excerpt || post.content_en,
  );
...
async function fetchBlogDetailUncached(
  slug: string,
  signal?: AbortSignal,
): Promise<BlogDetail> {
  ...
  const relatedPayload =
    isRecord(payload) && Array.isArray(payload.related) ? payload.related : [];
...
const mapAdminManagedBlogPost = (
  post: UnknownRecord,
  index = 0,
): AdminManagedBlogPost => ({
...
export async function fetchAdminBlogPosts(
  ...
  return {
    ...
    currentPage: isRecord(data)
      ? getNumber(data.current_page, filters.page || 1)
      : filters.page || 1,
...
export async function updateAdminBlogPost(
  ...
  const payload = await apiPut(
    `/admin/blog/${encodeURIComponent(String(postId))}`,
    body,
    signal,
  );
  const record = extractRecordPayload(payload);
...
export async function adminBlogQuickAction(
  ...
  await apiPost(
    `/admin/blog/${encodeURIComponent(String(postId))}/${action}`,
    {},
    signal,
  );
}

const mapSubscriptionPlan = (
  plan: UnknownRecord,
  index = 0,
): SubscriptionPlan => {
  ...
  return {
    ...
    description: getString(
      plan.description,
      "Start selling on Pyonea with tools for your store.",
    ),
    price: formatMMK(priceValue),
    priceValue,
    ...
    productsUsed:
      plan.products_used == null ? undefined : getNumber(plan.products_used),
  };
};

const mapSellerSubscription = (
  subscription: UnknownRecord,
): SellerSubscription => {
  const amountPaidValue = getNumber(subscription.amount_paid_mmk);
  const plan = isRecord(subscription.plan)
    ? mapSubscriptionPlan(subscription.plan)
    : null;
  ...
  return {
    ...
    statusLabel: getString(
      ...
      getString(subscription.status, "active").replaceAll("_", " "),
    ),
...
export async function fetchAdminSubscriptions(
  ...
  return {
    subscriptions: getArrayPayload(record)
      .filter(isRecord)
      .map(mapSellerSubscription),
...
export async function assignAdminSubscriptionPlan(
  ...
  const response = await apiPut(
    `/admin/subscriptions/${encodeURIComponent(String(userId))}`,
    {
...
export async function approveAdminSubscriptionRequest(
  ...
  const response = await apiPost(
    `/admin/subscriptions/requests/${encodeURIComponent(String(subscriptionId))}/approve`,
    {
      notes,
    },
    signal,
...
export async function rejectAdminSubscriptionRequest(
  ...
  const response = await apiPost(
    `/admin/subscriptions/requests/${encodeURIComponent(String(subscriptionId))}/reject`,
    {
      reason,
    },
    signal,
...
export async function updateAdminSubscriptionPlan(
  ...
  const response = await apiPut(
    `/admin/subscriptions/plans/${encodeURIComponent(String(planId))}`,
    {
...
const mapSellerWalletSummary = (wallet: UnknownRecord): SellerWalletSummary => {
  ...
  const codCommissionOutstandingValue = getNumber(
    wallet.cod_commission_outstanding,
  );
...
const mapSellerWalletTransaction = (
  ...
  const availableBalanceAfterValue = getNumber(
...
const mapSellerCodInvoice = (invoice: UnknownRecord): SellerCodInvoice => {
  ...
  return {
    ...
    invoiceNumber: getString(
      invoice.invoice_number,
      `INV-${getString(invoice.id)}`,
    ),
    orderId: getString(invoice.order_id),
    orderNumber: getString(
      order.order_number || invoice.order_number || invoice.order_id,
    ),
...
export async function fetchSellerWalletOverview(
  ...
  const walletData =
    isRecord(walletPayload) && isRecord(walletPayload.data)
      ? walletPayload.data
      : {};
  ...
  return {
    ...
    recentTransactions: transactions
      .filter(isRecord)
      .map(mapSellerWalletTransaction),
    invoices: getArrayPayload(invoicePayload)
      .filter(isRecord)
      .map(mapSellerCodInvoice),
  };
}
...
export async function fetchSellerCodInvoices(
  ...
  const payload = await apiGet(
    `/seller/cod-invoices?${query.toString()}`,
    signal,
  );
...
export async function submitSellerCodInvoicePayment(
  ...
  const response = await apiPost(
    `/seller/cod-invoices/${encodeURIComponent(String(invoiceId))}/submit-payment`,
    {
...
export async function fetchAdminCodInvoices(
  ...
  const invoicesSource =
    isRecord(data.invoices) && Array.isArray(data.invoices.data)
      ? data.invoices.data
...
export type AdminCommissionRuleType =
  | "default"
  | "account_level"
  | "category"
  | "business_type";
...
const mapAdminCommissionRule = (rule: UnknownRecord): AdminCommissionRule => {
  ...
  return {
    ...
    isActive:
      rule.is_active === true || rule.is_active === 1 || rule.is_active === "1",
  };
};
...
export async function updateAdminCommissionRule(
  ...
  await apiPut(
    `/admin/commission-rules/${encodeURIComponent(String(ruleId))}`,
    updates,
    signal,
  );
}
...
export async function fetchAdminCommissionCategories(
  ...
  return flattenAdminCommissionCategories(
    getArrayPayload(payload).filter(isRecord),
  );
}

export async function fetchAdminBusinessTypes(
  signal?: AbortSignal,
): Promise<AdminBusinessType[]> {
  ...
  return getArrayPayload(payload)
    ...
    .map((item) => ({
      ...
      name: getString(
        item.name || item.name_en || item.name_mm,
        "Business Type",
      ),
      slug: getString(item.slug || item.slug_en || item.slugEn || item.id),
      description: getString(
        item.description || item.description_en || item.description_mm,
      ),
...
const buildAdminCategoryFormData = async (
...
export async function fetchAdminManagedCategories(
  signal?: AbortSignal,
...
export async function createAdminCategory(
  ...
  await apiPostForm(
    "/admin/categories",
    await buildAdminCategoryFormData(payload, "create"),
    signal,
  );
}
...
const mapAdminManagedBusinessType = (
  item: UnknownRecord,
): AdminManagedBusinessType => ({
  id: getString(item.id),
  ...
  requiresRegistration:
    item.requires_registration === true || item.requires_registration === 1,
  requiresTaxDocument:
    item.requires_tax_document === true || item.requires_tax_document === 1,
  requiresIdentityDocument:
    item.requires_identity_document === true ||
    item.requires_identity_document === 1,
  requiresBusinessCertificate:
    item.requires_business_certificate === true ||
    item.requires_business_certificate === 1,
...
export async function updateAdminBusinessType(
  ...
  await apiPut(
    `/admin/business-types/${encodeURIComponent(String(businessTypeId))}`,
    payload,
    signal,
  );
}
...
export async function toggleAdminBusinessType(
  ...
  await apiPatch(
    `/admin/business-types/${encodeURIComponent(String(businessTypeId))}/toggle`,
    {},
    signal,
  );
}
...
const mapAdminEmailCampaign = (row: UnknownRecord): AdminEmailCampaign => ({
  ...
  openRate:
    row.open_rate == null || row.open_rate === ""
      ? null
      : getNumber(row.open_rate),
  sentAt: getString(row.sent_at),
  createdAt: getString(row.created_at || row.createdAt),
});

const mapAdminNewsletterSubscriber = (
  row: UnknownRecord,
...
const extractNewsletterMeta = (
  payload: unknown,
): AdminNewsletterSubscriberMeta => {
  const root = isRecord(payload) ? payload : {};
...
export async function fetchAdminEmailCampaigns(
  signal?: AbortSignal,
): Promise<AdminEmailCampaign[]> {
...
export async function updateAdminEmailCampaign(
  ...
  await apiPut(
    `/admin/newsletter/campaigns/${encodeURIComponent(String(campaignId))}`,
    body,
    signal,
  );
}
...
export async function fetchAdminNewsletterSubscriberStats(
  ...
  const payload = await apiGet(
    "/admin/newsletter/subscribers?per_page=1",
    signal,
  );
...
export async function fetchAdminNewsletterSubscribers(
  ...
  meta: AdminNewsletterSubscriberMeta;
}> {
...
const mapAdminAnalyticsStats = (stats: UnknownRecord): AdminAnalyticsStats => ({
  ...
  totalOrders: getNumber(stats.total_orders),
  completedOrders: getNumber(stats.completed_orders),
  paidCommissions: getNumber(
    stats.paid_commissions ?? stats.collected_commissions,
  ),
});

const mapAdminRevenueBreakdownRow = (
  row: UnknownRecord,
...
export async function fetchAdminAnalyticsStats(
  signal?: AbortSignal,
...
export async function fetchAdminRevenueBreakdown(
  ...
  return getArrayPayload(payload)
    .filter(isRecord)
    .map(mapAdminRevenueBreakdownRow);
}
...
export async function fetchSellerSubscriptionOverview(
  ...
  const currentData =
    isRecord(currentPayload) && isRecord(currentPayload.data)
      ? currentPayload.data
      : currentPayload;
  const plans = getArrayPayload(plansPayload)
    .filter(isRecord)
  ...
  return {
    ...
    pendingRequest: isRecord(pendingData)
      ? mapSellerSubscription(pendingData)
      : null,
    plans,
...
export async function createSellerSubscriptionPaymentSession(
  ...
  const payload = await apiPost(
    "/seller/subscription/payment-session",
    {
      plan_slug: planSlug,
      payment_method: paymentMethod,
    },
  ...
  return {
    ...
    deeplinkUrl: getString(
      data.deeplink_url || data.deep_link_url || data.deep_link,
    ),
...
export async function verifySellerSubscriptionPayment(
  ...
  const payload = await apiPost(
    "/seller/subscription/payment-session/verify",
    {
      payment_method: paymentMethod,
...
export async function upgradeSellerSubscription(
  ...
  return {
    message: isRecord(payload)
      ? getString(payload.message, "Subscription updated successfully.")
...
export async function fetchAdminReportDetail(
  ...
  const payload = await apiGet(
    `/admin/reports/${encodeURIComponent(ticketId)}`,
    signal,
  );
  const reportPayload =
    isRecord(payload) && isRecord(payload.data) ? payload.data : payload;
...
export async function fetchMyReportDetail(
  ...
  const payload = await apiGet(
    `/reports/${encodeURIComponent(ticketId)}`,
    signal,
  );
  const reportPayload =
    isRecord(payload) && isRecord(payload.data) ? payload.data : payload;
...
export async function addReportComment(
  ...
  await apiPost(
    `/reports/${encodeURIComponent(ticketId)}/comments`,
    { body },
    signal,
  );
}
