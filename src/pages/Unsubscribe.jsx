...
// One-click unsubscribe landing page — linked from every newsletter email.
...
const Unsubscribe = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await api.get(`/newsletter/unsubscribe?token=${encodeURIComponent(token)}`);
        if (r.data.success) {
          setStatus('success');
          setMessage(r.data.message || 'You have been unsubscribed.');
        } else {
          setStatus('error');
          setMessage(r.data.message || 'Something went wrong.');
        }
      } catch (e) {
        setStatus('error');
        setMessage(e.response?.data?.message || 'This unsubscribe link is invalid or has already been used.');
      }
    })();
  }, [token]);
          ...
          {status === 'success' && (
            <>
              ...
              <p className="text-gray-400 text-xs mt-2">
                You will no longer receive marketing emails from Pyonea. Transactional emails
                (order confirmations, password resets) are not affected.
              </p>
              <div className="pt-4 space-y-2">
                ...
                <button
                  onClick={async () => {
                    // Re-subscribe flow — redirect to subscribe
