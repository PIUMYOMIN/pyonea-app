    ...
    <View className="flex-row flex-wrap items-center gap-y-1">
      ...
      {showCount ? (
        <Text className="ml-2 font-sans text-sm text-gray-600 dark:text-slate-400 sm:text-base">
          ...
          {t("productDetail.reviews_count", { count: count || 0 })})
        </Text>
      ) : null}
    </View>
  );
...
function PillChip({
  ...
  return (
    <View
      className={`max-w-full rounded-full border px-2.5 py-1 ${containerClasses}`}
    >
      <Text
        className={`font-sans text-xs font-medium ${textClasses}`}
        numberOfLines={1}
      >
        {label}
      </Text>
...
function DeliveryTicker({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setAnimKeys((keys) => ({
        region:
          !prevLabel || nextLabel.region !== prevLabel.region
            ? keys.region + 1
            : keys.region,
        city:
          !prevLabel || nextLabel.city !== prevLabel.city
            ? keys.city + 1
            : keys.city,
        township:
...
export function ProductDetailNative({
  ...
  const [quantity, setQuantity] = useState(
    toPositiveInt(initialProduct?.moq ?? 1, 1),
  );
  ...
  useEffect(() => {
    ...
    if (sellerId) {
      ...
      const sellerKey =
        product.seller?.slug || String(product.seller?.id || sellerId);
  ...
  const availableStock = selectedVariant
    ...
    : (product?.stock ?? 0);
  const effectiveMoq = toPositiveInt(
    selectedVariant?.moq ?? product?.moq ?? 1,
    1,
  );
  ...
  const baseComparePrice =
    selectedVariant?.price || product?.originalPrice || product?.price || "";
  ...
  const sellerHref = product?.seller
    ? (mergeRouteLang(
        `/sellers/${product.seller.slug || product.seller.id}`,
        {},
        language,
      ) as Href)
  ...
  const handleAddToCart = async () => {
    ...
    if (hasVariants && !selectedVariant) {
      const requiredOption = product.options.find(
        (option) => option.isRequired,
      );
      setVariantError(
        requiredOption
          ? t("productDetail.select_required_option", {
              option: requiredOption.name,
            })
    ...
    try {
      ...
      showMessage(
        "success",
        result.message || t("productDetail.added_to_cart"),
      );
      return true;
    } catch (err) {
      showMessage(
  ...
  const handlePrimaryCta = async () => {
    if (hasVariants && !selectedVariant) {
      setVariantError(
        (prev) => prev || t("productDetail.select_options_before_continue"),
      );
      return;
    }
  ...
  const handleBuyNow = async () => {
    if (hasVariants && !selectedVariant) {
      setVariantError(
        (prev) => prev || t("productDetail.select_options_before_purchase"),
      );
      return;
    }
  ...
  const handleToggleWishlist = async () => {
    ...
    try {
      ...
      showMessage(
        ...
        t(
          added
            ? "productDetail.added_to_wishlist"
            : "productDetail.removed_from_wishlist",
        ),
      );
    } catch (err) {
      showMessage(
  ...
  const handleWriteReviewPress = () => {
    ...
    if (!isBuyer) {
      setReviewMessage({
        type: "error",
        text: t("productDetail.only_buyers_review"),
      });
      return false;
    }
    ...
    if (hasReviewedProduct) {
      setReviewMessage({
        type: "error",
        text: t("productDetail.already_reviewed"),
      });
      return false;
    }
  ...
  const handleSubmitReview = async (rating: number, comment: string) => {
    ...
    if (rating === 0) {
      setReviewMessage({
        type: "error",
        text: t("productDetail.select_rating_error"),
      });
      return false;
    }
    ...
    try {
      ...
      setReviewMessage({
        ...
        text: formatApiErrorMessage(
          err,
          t("productDetail.review_submit_failed"),
        ),
      });
      return false;
    } finally {
        ...
        <View className="grid min-w-0 grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-12">
            ...
            <ProductDetailGallery
              ...
              discountLabel={
                displayDiscountPct > 0
                  ? t("productDetail.discount_off", {
                      percent: Math.round(displayDiscountPct),
                    })
                  : undefined
              }
            />
          ...
          <View className="min-w-0 gap-6">
                ...
                <>
                  ...
                  {displayDiscountSaved ? (
                    <Text className="mt-0.5 font-sans text-sm font-medium text-green-600 dark:text-green-400">
                      {t("productDetail.you_save", {
                        amount: displayDiscountSaved,
                      })}
                    </Text>
                  ) : null}
                </>
            ...
            <View className="flex-row flex-wrap gap-2">
              <PillChip
                label={`${t("productDetail.moq")}: ${effectiveMoq} ${unitLabel}`}
              />
              ...
              {product.seller?.verified ? (
                <PillChip
                  label={t("productDetail.verified_seller")}
                  tone="verified"
                />
              ) : null}
              {!deliveryLoading && deliveryLabels.length > 0 ? (
                <PillChip
                  label={t("productDetail.delivery_zones_available")}
                  tone="delivery"
                />
              ) : null}
            </View>
                ...
                >
                  <View className="min-w-[520px] gap-2">
                    <View className="flex-row border-b border-amber-200 pb-1 dark:border-amber-800">
                      ...
                      <Text className="w-[30%] font-sans text-xs font-medium text-gray-500 dark:text-slate-400">
                        {t("productDetail.price_per_unit", {
        ...
        <View className="min-h-0 min-w-0 flex-1">
          ...
          {showMobileStickyCta ? (
            <View className="fixed bottom-0 left-0 right-0 z-40 border-t border-gray-200 bg-white/95 py-3 backdrop-blur dark:border-slate-800 dark:bg-slate-900/95">
              <View
                className={`${SITE_CONTAINER_CLASS} min-w-0 flex-row items-center gap-3`}
              >
                <View className="min-w-0 flex-1">
                  <Text
                  ...
                  <Text
                    className="font-sans text-sm font-bold text-gray-900 dark:text-slate-100"
                    numberOfLines={1}
                  >
                    {displayPrice}
                  </Text>
                ...
                {stockIsOut ? (
                  ...
                  >
                    ...
                    />
                    <Text
                      className="ml-1.5 font-sans text-sm font-semibold text-white"
                      numberOfLines={1}
                    >
                      {primaryCtaLabel}
                    </Text>
