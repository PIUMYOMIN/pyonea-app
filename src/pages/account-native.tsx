  ...
  return (
    ...
    >
      ...
      <View className="min-w-0 flex-1">
        <Text className={`font-sans text-sm font-semibold ${labelColor}`}>
          {label}
        </Text>
        {description ? (
          <Text
  ...
  return (
    ...
    >
      <View className="h-9 w-9 items-center justify-center rounded-xl bg-gray-100 dark:bg-slate-800">
        <Feather
          name={icon === "telegram" ? "send" : "message-circle"}
          size={18}
          color={iconColor}
        />
      </View>
...
export function AccountNative() {
  ...
  const handleSignOut = () => {
    ...
    if (Platform.OS === "web") {
      if (
        window.confirm(
          t("account_page.sign_out_confirm", {
    ...
    Alert.alert(
      ...
      [
        ...
        {
          text: t("account_page.sign_out", { defaultValue: "Sign Out" }),
          style: "destructive",
          onPress: runLogout,
        },
      ],
  ...
  const accountLinks = useMemo(() => {
    ...
    if (hasUserRole(user, "seller")) {
      links.push({
        key: "wallet",
        icon: "credit-card",
        label: t("account_page.seller_wallet", {
          defaultValue: "Seller Wallet",
        }),
      ...
      <View className="bg-gray-50 dark:bg-gray-900">
        <View className="mx-auto w-full max-w-lg gap-6 px-4 py-4">
            ...
            <View className="gap-4 rounded-2xl border border-gray-200 bg-white px-4 py-4 dark:border-slate-700 dark:bg-slate-900">
              <Text className="font-sans text-sm leading-6 text-gray-600 dark:text-slate-300">
                {t("account_page.guest_prompt", {
                  defaultValue:
                    "Sign in to access your dashboard, orders, and saved items.",
                })}
              </Text>
          ...
          {!auth.isLoading && user && accountLinks.length > 0 ? (
            <AccountSection
              title={t("account_page.section_account", {
          ...
          <AccountSection
            title={t("account_page.section_shopping", {
              defaultValue: "Shopping",
            })}
          >
            <AccountRow
              icon="package"
              label={t("account_page.track_order", {
            ...
            <AccountRow
              icon="tag"
              label={t("account_page.local_deals", {
                defaultValue: "Local Deals",
              })}
          ...
          <AccountSection
            title={t("account_page.section_preferences", {
              defaultValue: "Preferences",
            })}
          >
            <AccountRow
              icon={isDark ? "moon" : "sun"}
              label={t("account_page.appearance", {
                defaultValue: "Appearance",
              })}
              description={
                isDark
                  ? t("account_page.theme_dark", { defaultValue: "Dark mode" })
                  : t("account_page.theme_light", {
          ...
          <AccountSection
            title={t("account_page.section_live_chat", {
          ...
          >
            <ChatRow
              icon="telegram"
              iconColor="#229ED9"
              label={t("account_page.telegram_chat", {
            ...
            <ChatRow
              icon="whatsapp"
              iconColor="#25D366"
              label={t("account_page.whatsapp_chat", {
          ...
          <AccountSection
            title={t("account_page.section_support", {
              defaultValue: "Support & Contact",
            })}
          >
            <AccountRow
              icon="alert-circle"
              label={t("footer.report_issue", { defaultValue: "Report" })}
              onPress={() => router.push(href("/report"))}
            />
            <AccountDivider />
            <AccountRow
              icon="mail"
              label={t("account_page.email_support", {
            ...
            <AccountRow
              icon="phone"
              label={t("account_page.phone_support", {
                defaultValue: "Phone Support",
              })}
              description="+95 979 211 5547"
            ...
            <AccountRow
              icon="external-link"
              label={t("account_page.contact_us", {
          ...
          <AccountSection
            title={t("account_page.section_legal", { defaultValue: "Legal" })}
          >
            <AccountRow
              icon="file-text"
              label={t("account_page.terms", {
          ...
          {user ? (
            <AccountSection
              title={t("account_page.section_session", {
